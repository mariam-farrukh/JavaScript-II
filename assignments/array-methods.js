...
let ticketPriceTotal = runners.reduce(function(addedTotal, currentValue){
    return addedTotal + currentValue.donation;
  }, 0);
...
// Problem 1 Capitalize all the last names to print them onto jerseys
let lastAllCaps = runners.map(function(currentValue){
    return currentValue.last_name.toUpperCase();
});
console.log(lastAllCaps); 

// Problem 2 Compile a contact list of the runners Full name and email address
...
// Problem 3 Compile a list of poeople to thank at companies.
let thankPersonAtTheCompany = runners.filter(function(currentValue) {
    console.log(`Thank you, ${currentValue.first_name} ${currentValue.last_name} at ${currentValue.company_name}`);
});
console.log(thankPersonAtTheCompany);